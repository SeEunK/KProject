...
namespace WhatIsFunction
{
    internal class MineGame
    {
         ...
         * debug mode 에서 지뢰가 아닌곳은 .(닷), 지뢰인곳은 #(샵)으로 표현
         * play mode 에서 확인되지않은 곳은 전부 □(스퀘어) 로 표현
         * 첫 턴에 지뢰를 밟으면 해당 캄에 지뢰를 지워줌
         */
        public static void Main()
        {
             ...
             * - 빈칸 : MINE_PERCENTAGE 이상의 값
             * 
             * playBoard 상태
             * - -2: 지뢰 없음
             * - -1: 초기값
             * -  n: 주변 9타일 이내에 지뢰수 (0일 경우 ■ 표기, 양수일 경우 정수 표기)
             * 
             * mineCntMap 상태
             * - -1 : 지뢰 있음
             * -  n : 주변 0타일 이내에 지뢰 수
            ...
            while (isGameOver == false)
            {
                ...
                for (int y = 0; y < BOARD_SIZE_Y; y++)
                {
                    for (int x = 0; x < BOARD_SIZE_X; x++)
                    {
                        ...
                        {
                            case -2:
                                Console.Write("X".PadRight(3, ' ')); // PadRight
                                break;
                            case -1:
                ...
                while (isLocationValid == false)
                {
                    ...
                    //좌표를 제대로 입력한 경우만 이 아래로 코드가 진행됨. 왜냐면 유효하지않은 경우 위에서 continue 만나기 때문.

                    //플레이 보드에서 선택 가능한지 검사 한다
                ...
                for(int y = 0; y < BOARD_SIZE_Y; y++)
                {
                   //오픈 되지 않은 타일이 있는 경우 게임을 이어서 진행한다.
                   if(0< unknownTileCnt) { break; }
                    for (int x = 0; x < BOARD_SIZE_X; x++)
                    {
                        // 오픈되지않은 타일이 있는 경우 게임을 이어서 진행한다.
                        if (0 < unknownTileCnt) { break; }
                        ...
                        {
                            unknownTileCnt++;
                        } // if: 아직 오픈 할 타일이 존재하고, 해당 타일이 지뢰가 아닌경우.
                    }
